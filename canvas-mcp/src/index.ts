...
class CanvasAPIClient {
  ...
  async listCanvases(): Promise<Canvas[]> {
    return this.request<Canvas[]>("/canvases/");
  }

  async getCanvas(canvasId: number): Promise<Canvas & { nodes: CanvasNode[]; connections: NodeConnection[] }> {
  ...
  async listNodes(canvasId: number): Promise<CanvasNode[]> {
    return this.request<CanvasNode[]>(`/nodes/?canvas_id=${canvasId}`);
  }
  ...
  async createNode(
    ...
    return this.request<CanvasNode>("/nodes/", {
      method: "POST",
      body: JSON.stringify({
        canvas_id: canvasId,
        node_type: nodeType,
  ...
  async updateNode(
    nodeId: number,
    data: { name?: string; content?: string; position_x?: number; position_y?: number }
  ): Promise<CanvasNode> {
    return this.request<CanvasNode>(`/nodes/${nodeId}`, {
      method: "PUT",
  ...
  async createConnection(
    ...
    return this.request<NodeConnection>("/nodes/connections", {
      method: "POST",
      body: JSON.stringify({
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case "get_canvas": {
          const { canvas_id } = args as { canvas_id: number };
        ...
        case "create_canvas": {
          const { name: canvasName, description } = args as { name: string; description?: string };
        ...
        case "create_node": {
          const { canvas_id, node_type, name: nodeName, content, position_x, position_y } = args as {
            canvas_id: number;
            node_type: string;
            name: string;
            content?: string;
        ...
        case "update_node": {
          const { node_id, name: nodeName, content } = args as {
            node_id: number;
            name?: string;
            content?: string;
          };
          const updates: { name?: string; content?: string } = {};
          if (nodeName) updates.name = nodeName;
        ...
        case "connect_nodes": {
          const { source_node_id, target_node_id } = args as {
            source_node_id: number;
            target_node_id: number;
          };
        ...
        case "delete_connection": {
          const { connection_id } = args as { connection_id: number };
        ...
        case "create_okr_structure": {
          const { canvas_id, problem, objective, key_results } = args as {
            canvas_id: number;
            problem: string;
            objective: string;
            key_results: string[];
          ...
          const problemNode = await client.createNode(
            canvas_id,
            "problem",
            `Problem: ${problem}`,
            problem,
          ...
          // Create objective
          const objectiveNode = await client.createNode(
            canvas_id,
            "objective",
            `Objective: ${objective}`,
            objective,
          ...
          // Create key results
          const krNodes: CanvasNode[] = [];
          for (let i = 0; i < key_results.length; i++) {
            const kr = key_results[i];
            const krNode = await client.createNode(
              canvas_id,
              "keyresult",
              `KR${i + 1}: ${kr}`,
              kr,
